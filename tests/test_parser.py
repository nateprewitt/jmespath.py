...
from jmespath import ast, exceptions, parser, visitor
...
class TestErrorMessages(unittest.TestCase):
    ...
    def test_bad_unicode_string(self):
        ...
        error_message = re.compile(
            r'Bad jmespath expression: ' r'Invalid \\uXXXX escape.*\\uAZ12',
            re.DOTALL,
        )
...
class TestParserWildcards(unittest.TestCase):
    ...
    def test_root_wildcard(self):
        ...
        self.assertEqual(
            sorted(self.parser.parse('*.notfoo').search(data)),
            sorted(['notfoo']),
        )
...
class TestMergedLists(unittest.TestCase):
    ...
    def test_merge_with_indices(self):
        ...
        self.assertEqual(
            match, ["one", "three", "five", "seven", "nine", "ten"]
        )
