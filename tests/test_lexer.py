...
from jmespath.exceptions import EmptyExpressionError, LexerError
...
class TestRegexLexer(unittest.TestCase):
    ...
    def test_field(self):
        ...
        self.assert_tokens(
            tokens, [{'type': 'unquoted_identifier', 'value': 'foo'}]
        )
    ...
    def test_quoted_identifier(self):
        ...
        self.assert_tokens(
            tokens, [{'type': 'quoted_identifier', 'value': "foobar"}]
        )

    def test_json_escaped_value(self):
        tokens = list(self.lexer.tokenize('"\u2713"'))
        self.assert_tokens(
            tokens, [{'type': 'quoted_identifier', 'value': "\u2713"}]
        )
