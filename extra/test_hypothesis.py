...
from jmespath import exceptions, lexer, parser
from jmespath.functions import Functions

if sys.version_info[:2] == (2, 6):
    raise RuntimeError(
        "Hypothesis tests are not supported on python2.6. "
        "Use python2.7, or python3.3 and greater."
    )


JSON_NUMBERS = st.integers() | st.floats(allow_nan=False, allow_infinity=False)
    ...
    try:
        ...
        if expr:
            assert expr[e.lex_position] == e.token_value[0], (
                ...
                "Expression: %s\n%s != %s"
                % (expr, expr[e.lex_position], e.token_value[0])
            )
        return
...
@given(
    arg=st.lists(
        st.floats() | st.booleans() | st.text() | st.none(), min_size=1
    )
)
